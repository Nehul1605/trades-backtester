...
export default function IntegrationsPage() {
  ...
  const [exnessData, setExnessData] = useState({
    login: "",
    server: "",
    password: "",
    bridgeToken: "",
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem("isConnectedExness") === "true")
      setIsConnectedExness(true);
    if (localStorage.getItem("isConnectedMT5") === "true")
      setIsConnectedMT5(true);
  }, []);
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="flex-1 p-6 space-y-8 max-w-7xl mx-auto w-full">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground max-w-2xl">
              Connect your external accounts to automatically track P&L and
              order history.
            </p>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {/* MT5 Connection */}
          <Card
            className={`border-border/60 shadow-md flex flex-col hover:border-primary/30 transition-all overflow-hidden relative group ${isConnectedMT5 ? "ring-1 ring-primary/50" : ""}`}
          >
            ...
            <CardContent className="space-y-4 flex-1 relative z-10">
              ...
              <div className="rounded-lg bg-amber-500/5 border border-amber-500/20 p-4 flex items-start gap-3 mt-4">
                ...
                <p className="text-[11px] text-amber-500/80 leading-relaxed italic">
                  <strong>Coming Soon:</strong> This integration is part of our
                  upcoming Pro Plan. We will use secure investor access to sync
                  your data.
                </p>
              </div>
            </CardContent>
          ...
          {/* Exness Connection */}
          <Card
            className={`border-border/60 shadow-md flex flex-col hover:border-primary/30 transition-all overflow-hidden relative group opacity-90`}
          >
            <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity pointer-events-none">
            ...
            <CardContent className="space-y-4 flex-1">
              ...
              <div className="rounded-lg bg-amber-500/5 border border-amber-500/20 p-4 flex items-start gap-3 mt-4">
                <AlertCircle className="w-5 h-5 text-amber-500 shrink-0 mt-0.5" />
                <p className="text-[11px] text-amber-500/80 leading-relaxed italic">
                  <strong>Premium Feature:</strong> Linking your Exness account
                  via API token is being optimized for our Pro Plan launch. Stay
                  tuned!
                </p>
              </div>
            </CardContent>
        ...
        <div className="mt-12 text-center p-8 border-t border-border/40">
          ...
          <div className="max-w-2xl mx-auto grid gap-4 text-xs text-muted-foreground">
            <p>
              Does this platform have access to my funds?{" "}
              <span className="text-foreground ml-1">
                No. All integrations use read-only API methods. We cannot place
                trades or withdraw funds.
              </span>
            </p>
            <p>
              How often does history sync?{" "}
              <span className="text-foreground ml-1">
                Orders are synced every 5 minutes automatically.
              </span>
            </p>
          </div>
