...
import { getLocalDateString, loadImage } from "@/lib/utils";
...
export function TraderStats({ initialTrades, hideAccountSelector = false }: TraderStatsProps) {
  ...
  const handleExportStatsPDF = async () => {
    const data = [
      ["Total Closed Trades", String(stats.total)],
      ["Winning Trades", String(stats.wins.length)],
      ["Losing Trades", String(stats.losses.length)],
    ...
    doc.text(`TRADER STATISTICS REPORT (${period.toUpperCase()})`, textStartX, 26.5);

    doc.setFontSize(8);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(148, 163, 184); // slate-400
    doc.text(`Date Generated: ${new Date().toLocaleDateString()}`, 142, 21);
    doc.text(`Time Period: ${period.toUpperCase()}`, 142, 26.5);

    const headers = ["Metric Description", "Performance Value"];
    ...
    doc.save(`trader_stats_${period}_${new Date().toISOString().split("T")[0]}.pdf`);
  };
