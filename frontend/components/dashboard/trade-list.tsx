...
export function TradeList({ trades }: TradeListProps) {
  ...
  const handleExportPDF = async () => {
    ...
    doc.text("CLOSED TRADES JOURNAL REPORT", textStartX, 26.5);

    doc.setFontSize(8);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(148, 163, 184); // slate-400
    doc.text(`Date Generated: ${new Date().toLocaleDateString()}`, 142, 21);
    doc.text(`Total Closed Trades: ${closedTrades.length}`, 142, 26.5);
    ...
    doc.save(`closed_trades_report_${new Date().toISOString().split("T")[0]}.pdf`);
  };
