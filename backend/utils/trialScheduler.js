...
export const startTrialCheckScheduler = () => {
  const checkTrials = async () => {
    ...
    try {
          ...
          await sendEmail({
            ...
            html: `
              <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e4e4e7; border-radius: 8px;">
                ...
                <p>To avoid losing access to your trade logs, analytics, and console, you must either:</p>
                <ul style="padding-left: 20px;">
                  <li style="margin-bottom: 10px;"><strong>Upgrade to Premium:</strong> <a href="https://tradetrackerpro.in/premium" style="color: #eab308; font-weight: bold; text-decoration: underline;">Upgrade Here ($8.99/mo)</a></li>
                  <li style="margin-bottom: 10px;"><strong>Verify Partner Broker:</strong> Submit your details inside the verification portal</li>
                </ul>
      ...
      for (const user of expiredUsers) {
        try {
          ...
          await sendEmail({
            ...
            text: `Hello ${user.name},\n\nYour 10-day promotional trial access has ended. Your dashboard and platform features are now locked.\n\nTo restore your logs and unlock full access to Live Market Stream, Operator Signals, Economic Calendar, and Calculators, please upgrade to Premium or submit your broker details for free verification.\n\nUnlock Dashboard: https://tradetrackerpro.in/premium\n\nBest regards,\nThe TradeTracker Pro Team`,
            html: `
              <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e4e4e7; border-radius: 8px;">
                <h2 style="color: #ef4444; margin-top: 0;">❌ Trial Ended</h2>
                <p>Hello <strong>${user.name}</strong>,</p>
                <p>Your 10-day promotional trial access has ended. Your dashboard and platform features are now locked.</p>
                <p>To restore your logs and unlock full access to Live Market Stream, Operator Signals, Economic Calendar, and Calculators, please upgrade to Premium or submit your broker details for free verification.</p>
      ...
      // 3. Ensure ALL users with expired promo trials have status set to "pending" in DB
      const expiredPromoUsersToLock = await User.find({
        isPromoUser: true,
        promoExpiresAt: { $lte: now },
        status: "approved",
        role: { $ne: "admin" },
      ...
      for (const user of expiredPromoUsersToLock) {
        const isVerified = await VerificationRequest.findOne({ user: user._id, status: "approved" });
        if (!isVerified) {
          user.status = "pending";
          await user.save();
