          ...
          <CardContent className="flex-grow pb-8">
            <div className="grid grid-cols-7 gap-3 h-full">
              {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
                <div
                  key={day}
                  className="text-center text-[11px] font-black uppercase text-muted-foreground/60 pb-4 h-fit tracking-widest"
                >
                  {day}
                </div>
              ))}
              {calendarData.days.map((day, i) => (
                ...
                >
                  {day && (
                    <>
                      <span className="text-[11px] font-black absolute top-3 left-3 text-muted-foreground/30 group-hover:text-primary transition-colors">
                        {day.day}
                      </span>
                      <div className="flex flex-col items-center gap-1.5">
                        <span
                          className={`text-base font-black tracking-tighter ${day.pnl && day.pnl > 0 ? "text-emerald-500" : day.pnl && day.pnl < 0 ? "text-rose-500" : "text-muted-foreground/10"}`}
                        >
                          {day.pnl
                            ? `${day.pnl > 0 ? "+" : ""}${Math.round(day.pnl).toLocaleString()}`
                            : "—"}
                        </span>
                        {day.pnl !== 0 && (
                          <div
                            className={`h-1 w-8 rounded-full ${day.pnl > 0 ? "bg-emerald-500/40" : "bg-rose-500/40"}`}
                          />
                        )}
                      </div>
                      {day.tradesCount > 0 && (
        ...
        <Card className="lg:col-span-1 bg-card/40 border-border/50 backdrop-blur-md flex flex-col">
          ...
          <CardContent className="space-y-4 flex-grow">
            {calendarData.weeks.map((week, idx) => (
              <div
                key={idx}
                className="p-3 rounded-xl bg-muted/20 border border-border/10 flex items-center justify-between"
              >
                <div>
                  <p className="text-[9px] font-bold text-muted-foreground uppercase">
                    {week.label}
                ...
                <div
                  className={`text-sm font-black ${week.pnl >= 0 ? "text-emerald-500" : "text-rose-500"}`}
                >
                  {week.pnl >= 0 ? "+" : ""}$
                  {Math.round(week.pnl).toLocaleString()}
                </div>
              </div>
            ))}
